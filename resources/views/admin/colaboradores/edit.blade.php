...
@section('title', 'Editar colaborador — ' . $sponsor->name)

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.sponsors.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Volver a colaboradores</a>
    <h2 class="text-2xl font-bold text-gray-900 mt-2">{{ $sponsor->name }}</h2>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-semibold text-gray-800">Datos del colaborador</h3>
        </div>
        <form method="POST" action="{{ route('admin.sponsors.update', $sponsor) }}" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-400">Subir nueva imagen reemplazará la actual.</p>
                @error('logo')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-100">
            <h3 class="font-semibold text-gray-800">Contenido (Stories)</h3>
            <p class="text-xs text-gray-400 mt-0.5">Imágenes y vídeos que se muestran en la app</p>
        </div>

        {{-- Upload form --}}
        <div class="p-6 border-b border-gray-100">
            <form method="POST" action="{{ route('admin.sponsors.stories.store', $sponsor) }}" enctype="multipart/form-data" class="flex items-end gap-3">
                @csrf
                <div class="flex-1">
